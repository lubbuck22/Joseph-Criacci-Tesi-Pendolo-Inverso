...
def gym_learning(env, high_PWM, low_PWM, step):
    ...
    if high_PWM <= 125 and low_PWM >= 50 and high_PWM >= low_PWM:
        if (not os.path.exists(model_path)):
            ...
            del model
        gym_learning(env, high_PWM - step, low_PWM)
        gym_learning(env, high_PWM, low_PWM + step)
    

cart_env = CartPoleEnv()
gym_learning(env=cart_env, high_PWM=110, low_PWM=70, step=5)
print(chr(27) + "[2J")
